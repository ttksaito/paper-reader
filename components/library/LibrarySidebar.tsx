...
export default function LibrarySidebar({ isOpen, onClose, onOpen, onPaperSelect, selectedPaperId }: LibrarySidebarProps) {
  ...
  return (
    <>
      ...
      {/* Library Sidebar - 幅が変化する単一コンポーネント */}
      <div className={`fixed left-0 top-0 bottom-0 bg-white border-r border-gray-200 z-50 flex flex-col transition-all duration-300 ${
        isOpen ? 'w-96 shadow-2xl' : 'w-16 shadow-md'
      }`}>
        {!isOpen ? (
          // アイコンバー表示
          <div className="flex flex-col items-center py-4 gap-3 h-full">
              ...
              <button
                onClick={() => {
                  console.log('Library open button clicked');
                  onOpen();
                }}
